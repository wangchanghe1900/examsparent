layui.use(['form','layer','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        elem: '#resourceList',
        url :'getCommResourceList?resourceType=2',
        cellMinWidth : 95,
        page : true,
        height : "full-100",
        limits : [10,15,20,25],
    ...
    //编辑视频信息
    function editAudioResource(edit){
        var index = layui.layer.open({
            title : "编辑资源",
            type : 2,
            content : "editAudioResourceList",
    ...
    //添加用户
    function addAudioResource(){
        var index = layui.layer.open({
            title : "新增视频资源",
            type : 2,
            content : "audioResourceAdd",
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                setTimeout(function(){
                    layui.layer.tips('点击此处返回资源列表', '.layui-layer-setwin .layui-layer-close', {
    ...
    $(".delAll_btn").click(function(){
        var checkStatus = table.checkStatus('audioListTable'),
    ...
    function audioList(data) {
        layer.open({
            ...
            closeBtn: 0,
            shadeClose: true,
            content : "resource/audioDetailView",
            success : function(layero, index){
