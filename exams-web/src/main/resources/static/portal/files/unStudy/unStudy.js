...
function loadcontent( pagenum,myInfo){
	...
	if(pagenum==0){
		...
		loadStudyData(code,"/examsweb/api/resource/unlearnedresource",0,0);
	}else{
 	    ...
 	    loadUpFn : function(me){
 			...
 			if($(dropElement).attr("id")=="nostudyDropload"){
 				...
 				if($(dropElement).attr("id")=="studyDropload"){
 	 				loadStudyData(code,"/examsweb/api/resource/unlearnedresource",1,0);
 	 			}
 			}
 			// 为了测试，延迟1秒加载
            setTimeout(function(){
 	    ...
 	    loadDownFn : function(me){
 	    	...
 	    	if($(dropElement).attr("id")=="nostudyDropload"){
 	    		...
 	    		if(((unstudypageNum -1) * showNum) < unstudytotal){
	 	    		...
	 	    		urlstr="/examsweb/api/resource/unlearnedresource"
	 	 	    	//参数
	 	 			var code="";
	 	    		//{"empID"："","showNum":10,"pageNum":1}
