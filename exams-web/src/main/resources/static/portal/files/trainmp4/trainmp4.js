...
$(function(){
	...
	document.getElementById("container1").style.height = windowHeiht + "px";  
	document.getElementById("container1").style.paddingTop = 50/ 1334 * windowHeiht + "px";  
	
	var len = document.getElementsByClassName("video-imgbg").length;  
	for (i = 0; i < len; i++) {    
		...
		document.getElementsByClassName("video-imgbg").item(i).style.width =  windowWidth + "px";      
		document.getElementsByClassName("video-imgbg").item(i).style.height =  windowHeiht + "px";  
	}
	
	var src=GetQueryString("file");
	//alert(src);
	$("#container1 source").prop("src",src);
	var materialimg = getCookie("materialimg");
	$("#container1 video").css("background","transparent url('"+materialimg+"') 50% 50% no-repeat");
