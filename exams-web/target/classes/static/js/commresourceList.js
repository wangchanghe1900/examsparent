layui.use(['form','layer','table','laytpl'],function(){
    ...
    //显示页面按钮
    $.get(webpath+"/sysmenu/btnAuthroInfo",{"btn":"commres"},
        function (data) {
            if(data){
                if(data.isFind){
    ...
    var tableIns = table.render({
        elem: '#resourceList',
        url :'getCommResourceList?resourceType=0',
        cellMinWidth : 95,
        page : true,
        height : "full-100",
        limits : [10,15,20,25],
    ...
    //编辑用户
    function editCommResource(edit){
        var index = layui.layer.open({
            title : "编辑资源",
            type : 2,
            content : "editCommResourceList",
    ...
    table.on('tool(resourceList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            window.sessionStorage.setItem("pdfUrl",data.url);
            pdfList(data);
        }
    });
